...
protected:
    ...
    bool Fill() {
        uint64_t pos = nSrcPos % vchBuf.size();
        uint64_t readNow = vchBuf.size() - pos;
        uint64_t nAvail = vchBuf.size() - (nSrcPos - nReadPos) - nRewind;
        if (nAvail < readNow)
            readNow = nAvail;
        if (readNow == 0)
            return false;
...
public:
    ...
    bool jumpToEof() {
        ...
        uint64_t nPos = (uint64_t)ftell(src);
        nSrcPos = nPos;
        nReadPos = nPos;
    ...
    void read(char *pch, size_t nSize) {
        ...
        while (nSize > 0) {
            if (nReadPos == nSrcPos)
                Fill();
            uint64_t pos = nReadPos % vchBuf.size();
            size_t nNow = nSize;
    ...
    bool Seek(uint64_t nPos) {
        ...
        if (fseek(src, nLongPos, SEEK_SET))
            return false;
        nLongPos = ftell(src);
        nSrcPos = (uint64_t)nLongPos;
        nReadPos = (uint64_t)nLongPos;
        return true;
    }
    ...
    template<typename T>
    CBufferedFile& operator<<(const T& obj)
    {
        // Serialize to this stream
        ::Serialize(*this, obj);
        return (*this);
    ...
    void write(const char* pch, size_t nSize)
    {
        if (fwrite(pch, 1, nSize, src) != nSize)
            throw std::ios_base::failure("CBufferedFile::write: write failed");
    }
};
